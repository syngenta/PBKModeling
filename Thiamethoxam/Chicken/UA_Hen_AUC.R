...
SA_AUC_chicken          <- SA_AUC[SA_AUC$Species == 'Chicken',]
SA_AUC_chicken   <- SA_AUC_chicken   %>%
...
print(SA_AUC_chicken$parameter)
'[1] "Clint_parent"           "Clint_daughter"         "Rblood2plasma_parent"   "Rblood2plasma_daughter" "fup_parent"            
[6] "fup_daughter"           "Qgfr_daughter"  '
file_path     <- file.path(current_dir, "HTTK", "Bird", "TMX", "Laying hen")
write.csv(SA_AUC_chicken, paste(file_path, '/SA_AUC_chicken.csv', sep = ''), row.names = FALSE)
...
source(file.path("HTTK", "Bird", "TMX", 'Laying hen', 'UASupporting_Laying_hen.R'))
set.seed(10) 
SPECIES  <- 'Laying_hen'
...
fraction_daughter            <-  1
pars  <-  c( ka                   = ka,                 
             Clint_parent         = 259.3772, 
             Clint_daughter       = 41.00266,   
...
for (j in seq(UA_list)){
  ...
  for (i in 1:N_refined){
    ...
    MC.AUC[i,j]     <- max(df$C_blood_parent + df$C_blood_daughter)
...
file_path     <- file.path(current_dir, "HTTK", "Bird", "TMX", "Laying hen")
write.csv(stat, paste(file_path, '/UA_AUC_chicken.csv', sep = ''), row.names = FALSE)
