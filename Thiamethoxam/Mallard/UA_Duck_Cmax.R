...
SA_Cmax_duck          <- SA_Cmax[SA_Cmax$Species == 'Duck',]
SA_Cmax_duck   <- SA_Cmax_duck   %>%
...
print(SA_Cmax_duck$parameter)
file_path     <- file.path(current_dir, "HTTK", "Bird", "TMX", "Duck")
write.csv(SA_Cmax_duck, paste(file_path, '/SA_Cmax_duck.csv', sep = ''), row.names = FALSE)
...
file_path     <- file.path(current_dir, "HTTK", "Bird", "TMX", "Duck")
write.csv(stat, paste(file_path, '/UA_Cmax_duck.csv', sep = ''), row.names = FALSE)
