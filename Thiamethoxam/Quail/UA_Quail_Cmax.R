...
SA_Cmax_quail          <- SA_Cmax[SA_Cmax$Species == 'Quail',]
SA_Cmax_quail   <- SA_Cmax_quail   %>%
...
print(SA_Cmax_quail$parameter)
file_path     <- file.path(current_dir, "HTTK", "Bird", "TMX", "Quail")
write.csv(SA_Cmax_quail, paste(file_path, '/SA_Cmax_quail.csv', sep = ''), row.names = FALSE)
...
ka.mean                            <- 12.1847922
Kmuscle2pu_daughter.mean           <- 1.331
Vmuscle.mean                       <- 0.49
...
file_path     <- file.path(current_dir, "HTTK", "Bird", "TMX", "Quail")
write.csv(stat, paste(file_path, '/UA_Cmax_quail.csv', sep = ''), row.names = FALSE)
